...
export default function PsychologistMap({ 
  ...
  return (
    <div className="h-full w-full relative">      <MapContainer
        center={mapCenter}
        zoom={12}
      ...
      >
        ...
        <div className="leaflet-top leaflet-right" style={{zIndex: 1000}}>
          <div className="leaflet-control-zoom leaflet-bar leaflet-control shadow-md rounded-md overflow-hidden">
            <a className="leaflet-control-zoom-in bg-white hover:bg-gray-100 text-lg font-medium flex items-center justify-center h-8 w-8" 
               href="#" title="Zoom in" role="button" aria-label="Zoom in">+</a>
            <a className="leaflet-control-zoom-out bg-white hover:bg-gray-100 text-lg font-medium flex items-center justify-center h-8 w-8 border-t border-gray-100" 
               href="#" title="Zoom out" role="button" aria-label="Zoom out">−</a>
          </div>
        </div>
        
        <MapUpdater center={mapCenter} selectedPsychologist={selectedPsychologist} />
        ...
        {psychologists.map((psychologist) => (          <Marker
            key={psychologist.id}
                ...
                <div className="mt-3 space-y-2">
                  ...
                  {typeof psychologist.experience === 'number' && (
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{psychologist.experience} years</p>
                    </div>
                  )}
                  ...
                  {typeof psychologist.price === 'number' && (
                    ...
                    <span className={`px-2 py-1 rounded-full text-xs font-medium transition-all ${
                      ...
                      <span className={`inline-block w-2 h-2 rounded-full mr-1 ${
                        typeof psychologist.available === 'boolean'
                          ? (psychologist.available ? 'bg-green-500' : 'bg-red-500')
                          : 'bg-gray-400'
                      }`}></span>
                      {typeof psychologist.available === 'boolean' 
      ...
      <div className="absolute bottom-4 left-4 bg-white p-4 rounded-lg shadow-lg z-[1000] border border-gray-100 animate-fadeIn">
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-2 p-1 hover:bg-gray-50 rounded transition-colors">
            ...
            <span className="text-xs text-gray-700">Available Psychologist</span>
          </div>
          {userLocation && (
            <div className="flex items-center gap-2 p-1 hover:bg-gray-50 rounded transition-colors">
              <div className="w-5 h-5 relative flex items-center justify-center">
