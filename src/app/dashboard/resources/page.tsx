...
export default function ResourcesPage() {
  ...
  const getAllRecommendations = (): RecommendationItem[] => {
    ...
    Object.entries(recommendationsData).forEach(
      ([condition, severityLevels]) => {
  ...
  const mapRecommendationsToResources = (): Resource[] => {
    return allRecommendations.map((rec) => {
      let type: "article" | "video" | "podcast" | "exercise" | "tool" =
        "article";
      ...
      if (
        ...
        rec.tags?.some((tag) =>
          ["cbt", "worksheet", "therapy", "technique"].includes(tag)
        )
      ) {
        type = "tool";
      } else if (
        rec.title.toLowerCase().includes("journal") ||
      ...
      if (rec.id.startsWith("anx")) {
        imageUrl =
          "https://images.unsplash.com/photo-1633526543814-9718c8922b7a";
      ...
      <div className="bg-background rounded-xl shadow-sm p-6 border border-muted mb-6">
        <div className="mb-6">
          ...
          <div className="mt-2 text-sm">
            <span className="font-medium">{getActiveConditionCount()}</span>{" "}
            resources available
          </div>
        </div>
        ...
        {filteredResources.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredResources.map((resource) => (
              <div
                key={resource.id}
                className="flex flex-col border border-muted rounded-lg overflow-hidden transition-all hover:shadow-md hover:border-primary/30 h-full bg-gradient-to-b from-background to-background/50"
              >
                <div className="h-40 relative bg-muted/30">
                  {resource.imageUrl ? (
                    <>
                      ...
                      >
                        ...
                        >
                          {" "}
                          {(() => {
                            const origRec = allRecommendations.find(
                              (r) => r.id === resource.id
                            );
                    ...
                    <div
                      className={`absolute inset-0 ${
                        resource.id.startsWith("anx")
                          ? "bg-gradient-to-br from-indigo-50 to-indigo-200"
                          : resource.id.startsWith("dep")
                  ...
                  <div className="absolute top-3 left-3">
                    ...
                    >
                      {resource.id.startsWith("anx")
                        ...
                        ? "Depression"
                        : "Stress"}
                      {resource.id.includes("min")
                        ? " - Minimal"
                        : resource.id.includes("mild")
                        ? " - Mild"
                        : resource.id.includes("mod") &&
                          !resource.id.includes("modsev")
                        ? " - Moderate"
                        : resource.id.includes("modsev")
                        ? " - Moderately Severe"
                        : resource.id.includes("sev")
                        ? " - Severe"
                        : resource.id.includes("low")
                        ? " - Low"
                        : resource.id.includes("high")
                        ? " - High"
                        : ""}
                    </span>
                  </div>
