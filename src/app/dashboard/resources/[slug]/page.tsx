...
const findRecommendationBySlug = (slug: string): RecommendationItem | null => {
  const allRecommendations: RecommendationItem[] = Object.values(
    recommendationsData
  ).flatMap((severityLevels) =>
    Object.values(severityLevels).flat()
...
export default function ResourcePage() {
  ...
  const findRelatedResources = (
    currentRec: RecommendationItem
  ): RecommendationItem[] => {
    ...
    return allRecommendations
      ...
      .filter((rec) =>
        rec.tags?.some((tag: string) => currentRec.tags?.includes(tag))
      )
      .sort((a, b) => {
        const aMatchCount =
          a.tags?.filter((tag: string) => currentRec.tags?.includes(tag))
            .length || 0;
        const bMatchCount =
          b.tags?.filter((tag: string) => currentRec.tags?.includes(tag))
            .length || 0;
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-3">
            <div>
              <span
                className={`inline-block px-3 py-1 rounded-full text-xs font-medium mb-4 ${
                  recommendation.id.startsWith("anx")
                    ? "bg-indigo-600 text-white"
                    : recommendation.id.startsWith("dep")
            ...
            <div
              className={`mt-4 md:mt-0 w-16 h-16 rounded-full flex items-center justify-center ${
                recommendation.id.startsWith("anx")
          ...
          <div className="prose prose-sm dark:prose-invert max-w-none">
                ...
                <div className="mt-2 flex items-center">
                  ...
                  <span
                    className={`inline-block px-2 py-1 rounded-full text-xs font-medium ${
                      recommendation.id.startsWith("anx")
                        ? "bg-indigo-600 text-white dark:bg-indigo-900 dark:text-indigo-100"
                        : recommendation.id.startsWith("dep")
                        ? "bg-blue-600 text-white dark:bg-blue-900 dark:text-blue-100"
                        : "bg-teal-600 text-white dark:bg-teal-900 dark:text-teal-100"
                    }`}
                  >
                    {recommendation.content.type.charAt(0).toUpperCase() +
                      recommendation.content.type.slice(1)}
                  </span>
            ...
            {recommendation.content ? (
              <>
                ...
                {/* Materials needed */}
                {recommendation.content.materials &&
                  recommendation.content.materials.length > 0 && (
                    <div className="mb-6">
                      <h4 className="font-medium">Materials Needed:</h4>
                      <ul className="list-disc pl-5 mt-2">
                        {recommendation.content.materials.map(
                          (material, index) => (
                ...
                {/* Steps */}
                {recommendation.content.steps &&
                  recommendation.content.steps.length > 0 && (
                    <div className="mb-6">
                ...
                {recommendation.content.toolUrl && (
                  <div className="mb-6">
                    <h4 className="font-medium mb-2">Interactive Tool</h4>{" "}
                    <Link
                      href={`/dashboard/coming-soon?tool=${encodeURIComponent(
                        recommendation.title + " Interactive Tool"
                      )}&return=${encodeURIComponent(
                        "/dashboard/resources/" + slug
                      )}`}
              ...
              <>
                ...
                <p>
                  This resource is designed to help you manage your mental
                  health. Follow the guidelines below to get the most benefit:
                </p>
                <ul>
                  ...
                  {relatedResources.map((resource) => (
                    <Link
                      ...
                      className={`p-4 border rounded-lg block ${
                        resource.id.startsWith("anx")
                          ? "border-indigo-900/30 hover:bg-indigo-500/10 "
                          : resource.id.startsWith("dep")
                          ? "border-blue-900/30 hover:bg-blue-500/10 "
                          : "border-teal-900/30 hover:bg-teal-500/10 "
                      }`}
                    >
