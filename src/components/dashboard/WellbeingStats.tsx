...
const WellbeingStats: React.FC<WellbeingStatsProps> = ({ className, onStreakUpdate }) => {
  ...
  const getMoodIcon = (mood: string) => {
    ...
    switch (lowerMood) {
      case 'joy':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-blue-500 dark:text-blue-400">
      ...
      case 'love':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" stroke="none" className="text-green-500 dark:text-green-400">
            <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
          </svg>
        );
      case 'surprise':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-purple-500 dark:text-purple-400">
            <circle cx="12" cy="12" r="10" />
      ...
      case 'neutral':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-gray-500 dark:text-gray-400">
            <circle cx="12" cy="12" r="10" />
      ...
      case 'fear':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-yellow-500 dark:text-yellow-400">
            <circle cx="12" cy="12" r="10" />
      ...
      case 'anger':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-red-500 dark:text-red-400">
            <circle cx="12" cy="12" r="10" />
      ...
      case 'sadness':
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-indigo-500 dark:text-indigo-400">
            <circle cx="12" cy="12" r="10" />
      ...
      default:
        return (
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="text-gray-500">
            <circle cx="12" cy="12" r="10" />
            <line x1="8" y1="15" x2="16" y2="15" />
            <line x1="9" y1="9" x2="9.01" y2="9" strokeWidth="3" />
            <line x1="15" y1="9" x2="15.01" y2="9" strokeWidth="3" />
  ...
  return (
    <div className={`${className || ''}`}>
      ...
      <div className="p-4 space-y-4">        {/* Wellness Score */}
        <div className="bg-muted/30 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium">Wellness Score</h3>
            <div className="flex items-center">
              <div className={`flex items-center justify-center w-12 h-12 rounded-full border-2 ${getWellnessScoreColor(stats.wellnessScore).replace('text-', 'border-')}`}>
                <span className={`text-xl font-bold ${getWellnessScoreColor(stats.wellnessScore)}`}>
                  {stats.wellnessScore}
                </span>
              </div>
            </div>
          ...
          <div className="h-2.5 w-full bg-muted rounded-full overflow-hidden">
            <div 
              className={`h-full transition-all duration-700 ease-in-out ${stats.wellnessScore >= 80 ? 'bg-green-500 dark:bg-green-400' : 
                stats.wellnessScore >= 60 ? 'bg-blue-500 dark:bg-blue-400' : 
                stats.wellnessScore >= 40 ? 'bg-yellow-500 dark:bg-yellow-400' : 
                stats.wellnessScore >= 20 ? 'bg-orange-500 dark:bg-orange-400' : 'bg-red-500 dark:bg-red-400'}`}
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="bg-muted/30 rounded-lg p-4 flex flex-col items-center justify-center">
            <div className="w-10 h-10 rounded-full bg-blue-200 flex items-center justify-center mb-2">
              <svg className="w-5 h-5 text-blue-600 dark:text-blue-400" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
            ...
            <span className="text-xs text-muted-foreground mb-1">Weekly Entries</span>
            <div className="flex items-center gap-1.5">
              <span className="text-2xl font-semibold">{stats.weeklyEntries}</span>
          ...
          <div className="bg-muted/30 rounded-lg p-4 flex flex-col items-center justify-center">
            ...
            <span className="text-xs text-muted-foreground mb-1">Current Streak</span>
            <div className="flex items-center">
            ...
            <div className="flex items-center mt-2 gap-0.5">
              {[...Array(7)].map((_, i) => (
                <div 
                  key={i} 
                  className={`h-1.5 w-3 rounded-sm ${i < stats.streakDays ? 'bg-orange-500 dark:bg-orange-400' : 'bg-muted'}`}
                ></div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-muted/30 rounded-lg p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="flex flex-col space-y-2">              {Object.entries(stats.moodDistribution)
                ...
                .sort((a, b) => b[1] - a[1])
                .slice(0, 3)
                .map(([mood, percentage]) => (                  <div key={mood} className="flex items-center gap-2">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="h-1.5 w-full bg-muted rounded-full overflow-hidden">
                        <div 
                          className={`h-full ${getMoodColor(mood)} transition-all duration-700 ease-in-out`}
            ...
            <div className="flex flex-col items-center justify-center bg-background rounded-lg p-3 border border-muted/40">
              <span className="text-xs text-muted-foreground mb-1">Dominant Mood</span>              <div className="w-12 h-12 rounded-full flex items-center justify-center bg-muted/20 mb-1">
              ...
              <div className={`w-6 h-1 mt-1 rounded-full ${getMoodColor(stats.dominantMood)}`}></div>
            </div>
          </div>
        </div>
          {/* Common Themes */}
