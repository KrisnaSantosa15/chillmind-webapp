...
//   onSave
  ...
  const getEmotionColor = (emotion: Emotion) => {
    switch(emotion) {
      ...
      case "love": return "bg-background border-green-400";
      case "surprise": return "bg-background border-purple-400";
      case "fear": return "bg-background border-yellow-400";
      case "anger": return "bg-background border-red-400";
      case "sadness": return "bg-background border-indigo-400";
      default: return "bg-background border-gray-400";
    }
  };
  const getEmotionAccent = (emotion: Emotion) => {
    switch(emotion) {
      case "joy": return "bg-blue-600";
      case "love": return "bg-green-600";
      case "surprise": return "bg-purple-600";
      case "fear": return "bg-yellow-600";
      case "anger": return "bg-red-600";
      case "sadness": return "bg-indigo-600";
      default: return "bg-gray-600";
    }
  };
  const getMoodBackground = (emotion: Emotion) => {
    switch(emotion) {
      case "joy": return "bg-blue-300";
      case "love": return "bg-green-300";
      case "surprise": return "bg-purple-300";
      case "fear": return "bg-yellow-300";
      case "anger": return "bg-red-300";
      case "sadness": return "bg-indigo-300";
    ...
    <AnimatePresence>
      {!isExiting && (
        <motion.div
          ...
          className={`p-5 rounded-xl border-2 shadow-md dark:shadow-lg dark:shadow-black/10 w-full my-4 ${getEmotionColor(emotion)}`}
        >          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center">
              <div className={`w-10 h-10 rounded-full ${getMoodBackground(emotion)} flex items-center justify-center mr-3 ${getMoodTextColor(emotion)}`}>
                {getMoodIcon(emotion)}
              </div>
              <h3 className="text-xl text-foreground dark:text-foreground font-semibold">{insight.title}</h3>
            </div><button 
            ...
            <button 
              ...
              className={`flex-1 px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                currentTab === 'learn' 
                  ...
                  : `text-foreground hover:bg-opacity-80 ${getMoodBackground(emotion)} bg-opacity-0`
              }`}
            >
          ...
          <div className=" rounded-lg p-4 shadow-sm min-h-[120px]">
            <AnimatePresence mode="wait">
              ...
              >
                {currentTab === 'learn' && (
                  <div>                    <p className="text-foreground">{insight.description}</p>
                    {insight.factoid && (
                  ...
                  <div>
                    <h4 className="text-sm font-semibold mb-2 text-gray-500 dark:text-gray-400">Try This Strategy</h4>
                    <p className="text-foreground">{insight.copingStrategy}</p>
                  </div>
                )}                {currentTab === 'reflect' && (
                  <div>
                    <h4 className="text-sm font-semibold mb-2 text-gray-500 dark:text-gray-400">For Reflection</h4>
                    <p className="text-foreground italic">{insight.reflectionPrompt}</p>
                  </div>
                )}
              </motion.div>
            </AnimatePresence>
