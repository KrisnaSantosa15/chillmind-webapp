    ...
    const fetchAssessmentData = async () => {
      ...
      try {
        ...
        if (assessment) {
          ...
          const depressionResult = getDepressionMetrics(
            predictionResults.depression.label
          );
          const anxietyResult = getAnxietyMetrics(
          ...
          const orderedMetrics = [
            {
              name: "Depression Level",
              value:
                (depressionResult.severityLevel / depressionResult.maxLevel) *
                100,
            ...
            },
            {
              name: "Anxiety Level",
              value:
                (anxietyResult.severityLevel / anxietyResult.maxLevel) * 100,
    ...
    <div>
          ...
          mentalHealthMetrics.map((metric) => (
            <div key={metric.name} className="mb-4">
              ...
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center">
                  {metric.severityLevel !== undefined &&
                    metric.maxLevel !== undefined && (
                      <span className="text-xs text-muted-foreground mr-2">
                        {metric.severityLevel}/{metric.maxLevel}
                      </span>
                    )}
                  <span
                    className={`text-xs font-medium px-2 py-1 rounded-full ${metric.colorClass.replace(
                      "bg-",
                      "bg-opacity-20 text-"
                    )}`}
                  >
      ...
      {loading ? (
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <Link
            href="/onboarding"
            className="px-4 py-2 text-sm font-medium text-center text-white bg-primary rounded-md hover:bg-primary/90 inline-block"
          >
            Take Assessment Again
          </Link>
          <Link
            href="/dashboard/assessments"
            className="px-4 py-2 text-sm font-medium text-center text-primary bg-primary/10 rounded-md hover:bg-primary/20 inline-block border border-primary/20"
          >
            View History
          </Link>
        </div>
