...
async function fetchGeminiResponseStream(prompt: string, onChunk: (chunk: string) => void): Promise<void> {
  try {
    ...
    onChunk("[Error: Unable to connect to AI service. Please try again later.]");
  }
}
    ...
    >
      ...
      <div className="p-4 border-b border-muted flex items-center justify-between bg-gradient-to-r from-primary/5 to-primary-light/5">
        <div className="flex items-center">
          ...
          <div>
            <h3 className="text-sm font-medium text-foreground">ChillMind Assistant</h3>
            <span className="text-xs text-primary flex items-center">
              <span className="h-1.5 w-1.5 rounded-full bg-green-500 mr-1"></span>
              {isTyping ? 
                <span className="mr-1">typing<span className="animate-pulse">...</span></span> : 
        ...
        <Link href="/dashboard/ai-assistant" className="text-xs text-primary hover:underline">
          Full View
        </Link>
      </div>
