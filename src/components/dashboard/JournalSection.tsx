      ...
      <div className={showHeader ? "mt-4" : ""}>
        ...
        <div className="flex justify-between mt-3">
          <button
            ...
            className={`ml-auto px-4 py-2 bg-primary text-white text-sm font-medium rounded-md hover:bg-primary/90 transition-colors flex items-center ${
      ...
      {showRecentEntries && recentEntries.length > 0 && (
        <div className="mt-6">
          ...
          <div className="flex justify-between items-center">
            ...
            {entriesLimit && recentEntries.length > entriesLimit && (
              <Link
          ...
          <div className="mt-3 space-y-3">
            {recentEntries
              .slice(0, entriesLimit || recentEntries.length)
              .map((entry) => (
                <div
                  key={entry.id}
                  className="journal-entry p-4 bg-muted rounded-lg"
                >
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      <div
                        className={`w-8 h-8 rounded-full ${getMoodBackground(
                          entry.mood
                        )} flex items-center justify-center ${getMoodTextColor(
                          entry.mood
                        )}`}
                      >
                  ...
                  <div className="mt-2 flex flex-wrap gap-2">
                    {entry.tags.map((tag, index) => (
                      <span
                        key={index}
                        className={`px-2 py-1 text-xs ${getTagStyle(
                          tag
                        )} rounded-full`}
                      >
                        #{tag}
